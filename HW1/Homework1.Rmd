...
tau2Start <- rep(1000,p)
betaStart <- solve(t(X)%*%X)%*%t(X)%*%y
sigma2Start <- t(y-X%*%beta)%*%(y-X%*%beta)/n
...
for(i in 1:length(lambdas)){
  ...
  run <- mcmc(samples, y, X, betaStart, tau2Start)
  lambda.chain <- run$lambda.chain
  sigma2.chain <- run$sigma2.chain
  tau2.chain <- run$tau2.chain
  beta.chain <- run$beta.chain
  beta.chain <- matrix(beta.chain,p,samples)
