...
lambda2 <- 1
for(s in 2:samples){
  #lambda2.p <- rgamma(1,1,10)
  lambda2.p <- gamma(1,lambda2,.5)
  logr <- sum(dgamma(tau2,1,lambda2.p/2,log=T))+
    ...
    dnorm(lambda2.p,lambda2,0.05)+
    dnorm(lambda2,lambda2.p, 0.05,log = T)
  if(log(runif(1))<logr){lambda2 <- lambda2.p}
  for(i in 1:p){
    tau2i.p <- runif(1,min=tau2[i]-tau2[i]/2,max=tau2[i]+tau2[i]/2)
    while(tau2i.p<=0){
      tau2i.p <- runif(1,min=tau2[i]-tau2[i]/10,max=tau2[i]+tau2[i]/10)
    }
    logr <- dnorm(beta[i],0,tau2i.p,log=T)+
      ...
      dunif(tau2i.p,min=tau2[i]-1, max=tau2[i]+1,log = T)+
      dunif(tau2[i],min=tau2i.p-1, max=tau2i.p+1,log = T)
    if(log(runif(1))<logr){tau2[i] <- tau2i.p}else{hey <- hey+1}
  }
  # sigma2.p <- runif(1, min=sigma2-0.1,max=sigma2+0.1)
  # while(sigma2.p<=0){
  #   sigma2.p <-runif(1, min=sigma2-.05,max=sigma2+.05)
  # }
  # logr <- dmvnorm(y,X%*%beta,sigma2.p*diag(n), log=T)+
  #   dgamma(1/sigma2.p,10,1/10,log=T)-
  #   dmvnorm(y,X%*%beta,sigma2*diag(n), log=T)-
  #   dgamma(1/sigma2,0.1,1/10,log=T)
  # if(log(runif(1))<logr){sigma2 <- sigma2.p}
  sigma2 <- rinvgamma(1,shape=n/2+0.1,scale=(0.1+t(y-X%*%beta)%*%(y-X%*%beta)/2))
...
fit <- glmnet(X[,-1], y)
df <- cbind(beta.ls,rowMeans(beta.keep[,floor(samples/4):samples]), coef(fit,s=0))
colnames(df) <- c("Least Square", "Bayes", "glmnet")
df
plot(lambda2.keep[1:samples], type="l")
beta.keep <- t(beta.keep)
tau2.keep <- t(tau2.keep)
tau2.keep <- scale(tau2.keep[1:samples,], center = FALSE, scale =colSums(tau2.keep[1:samples,]))
