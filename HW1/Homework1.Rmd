...
X <- cbind(rep(1,n),cbind(diabetes$x)); y <- diabetes$y; n <- nrow(X); p <- ncol(X)
samples <- 50000;
lambda <- 1
tau2 <- rep(1000,p)
beta <- solve(t(X)%*%X)%*%t(X)%*%y
sigma2.chain <- lambda.chain <- rep(0,samples)
beta.chain <- tau2.chain <- matrix(NA,nrow=p,ncol=samples)
sigma2 <- var(y)
sigma2.chain[1] <- sigma2
beta.chain[,1] <- solve(t(X)%*%X)%*%t(X)%*%y
...
fit <- glmnet(X, y)
glmnetCoef <- coef(fit,s=1)
# df <- cbind(beta.ls,colMeans(beta.chain[floor(samples/4):samples,]),glmnetCoef[-1,])
# colnames(df) <- c("Least Square", "Bayes", "Glmnet")
# df
plot(log(lambda.chain[1:samples]), type="l")
beta.ls <- solve(t(X)%*%X)%*%t(X)%*%y
cbind(beta.ls,betaChain)
beta.chain <- t(beta.chain)
tau2.chain <- t(tau2.chain)
plot(sigma2.chain[1:samples],type="l")
plot(log(tau2.chain[,1]),type="l")
plot(log(tau2.chain[,2]),type="l")
plot((tau2.chain[,3]),type="l")
plot((tau2.chain[,4]),type="l")
plot((tau2.chain[,5]),type="l")
plot((tau2.chain[,6]),type="l")
plot((tau2.chain[,7]),type="l")
```
\newpage
...
post.means[,index] <- matrix(rowMeans(beta.chain[,floor(samples/4):samples]),nrow = p)
index <- index +1
}
fit <- glmnet(X, y)
glmnetCoef <- coef(fit,s=1)
...
```{r, eval=F,warning=F}
bs <- c(seq(from=0.001,to=1,length.out = 10),seq(from=1.5,to=20,length.out = 10))
set.seed(1)
data("diabetes")
X <- cbind(diabetes$x); y <- diabetes$y; n <- nrow(X)
X <- cbind(rep(1,n),X)
...
tau2.chain[,1] <- tau2
s <- 2
lambdas <- seq(from=-12, to = -2, length.out = 6)
post.means <- matrix(NA,nrow=p,ncol=length(lambdas))
index <- 1
bindex <- 1
for(b in bs){
for(s in 2:samples){
  lambda.p <- exp(log(lambda)+rnorm(1,0,1))
  # logr <- loglambdaTarget(lambda=lambda.p,tau2=tau2,a=1,b=b)-loglambdaTarget(lambda=lambda,tau2=tau2,a=1,b=b)+log(lambda.p)-log(lambda)
  # if(log(runif(1))<logr){lambda <- lambda.p}
  lambda <- lambdaDraw(proposed=lambda.p, current=lambda,tau2=tau2,a=1,b=b)
  lambda.keep[s] <- lambda
  # for(j in 1:length(tau2)){
  #   tau2j.p <- exp(log(tau2[j])+rnorm(1,0,5))
  #   logr <-
  #     logtau2jTarget(tau2j.p, lambda, beta[j]) -
  #     logtau2jTarget(tau2[j], lambda, beta[j]) +
  #     log(tau2j.p)-log(tau2[j])
  #   if(log(runif(1))<logr){tau2[j] <- tau2j.p}
  # }
  tau2 <- tau2Draw(current=tau2,beta=beta, lambda=lambda)
...
betasB[,bindex] <- rowMeans(beta.chain[,1000:samples])
bindex <- bindex+1
}
...
# colnames(df) <- c("Least Square", "Bayes", "Glmnet")
# df
plot(bs,betasB[2,],ylim = c(min(betasB),max(betasB)),type="l", main="Effect of b", ylab="Coefficients",xlab="b's")
...
library(Rcpp)
library(rbenchmark)
loglambdaTarget(lambda = 1,tau2 = c(1,2,3),a=1,b=1)
loglambda.target(lambda = 1,tau2 = c(1,2,3),a=1,b=1)
logtau2jTarget(1,1,1)
logtau2j.target(1,1,1)
tau2j <- betaj <- lambda <- 2
(-log(sqrt(tau2j)) - 1/2*(1/tau2j*betaj^2 + lambda^2*tau2j))
```
